...
Implementar a funcionalidade de listagem e detalhamento de Pokémon no sistema.
...
Criar endpoint para listagem paginada de Pokémon.

A listagem deve suportar filtros por:

- nome;
- ordem;
- status;
- tipo, quando houver tipos cadastrados.

A resposta deve seguir o padrão de paginação já existente no projeto.

### 2.2 Sincronização inicial

Antes de responder a listagem, o backend deve garantir que a base local possui os registros básicos de Pokémon.

Cenários:

1. **Base vazia**  
   Buscar dados básicos na PokéAPI e salvar localmente como Pokémon incompletos.

2. **Base parcial**  
   Comparar total local com total externo e importar apenas Pokémon ausentes.

3. **Base completa**  
   Retornar dados da base local/cache.

Endpoint externo de referência:
...
- salvar `name`;
- salvar `url`, se existir no model;
- obter `order` usando `ensure_order_number(url)`;
- obter `external_image` usando `ensure_external_image(order)`;
- salvar `status` como `INCOMPLETE`;
- usar os valores default já existentes no domínio/model/schema para os demais campos.

Não duplicar regras de default se elas já existirem no model.

### 2.3 Cache Redis

Implementar cache respeitando os padrões existentes no projeto.
...
O cache `pokemon:meta` deve armazenar:

- total de Pokémon na base local;
- total de Pokémon informado pela PokéAPI.

TTL:

- maior que 2 horas;
- preferencialmente configurável por constante ou variável de ambiente.

Sempre invalidar/atualizar o cache após importações ou atualização de Pokémon.

### 2.4 Detalhamento

Criar endpoint para detalhar Pokémon.

Ao buscar um Pokémon:

- se estiver com `status = INCOMPLETE`, completar os dados usando a PokéAPI;
- se já estiver completo, retornar dados locais/cache.

Endpoint externo de referência:

https://pokeapi.co/api/v2/pokemon/{name}

Também buscar dados complementares necessários para:
...
- estatísticas;
- cadeia evolutiva;
- fraquezas e vantagens contra tipos, se aplicável ao domínio existente.

Após completar os dados, atualizar o status para completo conforme enum existente no projeto.
...
Criar página autenticada de listagem de Pokémon.

A página deve conter:

- cards de Pokémon;
- paginação;
- filtros por nome, ordem, status e tipo;
- clique no card redirecionando para a página de detalhe.

A implementação deve seguir a estrutura já usada em:

machado-web/app/ui/features/pokemon/list/usePokemonList.ts

Criar estrutura equivalente para detalhe, por exemplo:

machado-web/app/ui/features/pokemon/detail/usePokemonDetail.ts
...
- altura;
- peso;
- taxa/pontos de captura;
- habilidades;
- movimentos;
- tipos;
- fraquezas;
- vantagens;
- estatísticas com barras horizontais;
- timeline de evoluções.

### 3.3 Badges de tipo

Os tipos devem ser renderizados usando o objeto retornado pela API:
...
A seção de movimentos deve:

- exibir inicialmente os 2 primeiros movimentos;
- ter botão "ver mais";
- a cada clique, exibir mais 2 movimentos.

---

## 4. Regras importantes

- Não criar arquitetura nova se já existir padrão no projeto.
- Não alterar nomes de atributos existentes sem necessidade.
- Os exemplos de response são apenas referência.
- Se algum campo esperado não existir no domínio atual, usar o campo equivalente existente ou propor ajuste mínimo.
- Respeitar os padrões de services, repositories, schemas, hooks e components já existentes.
- Evitar duplicação de regra entre backend e frontend.
...
- a listagem de Pokémon funcionar com paginação;
- os filtros funcionarem;
- a base local for populada automaticamente quando vazia;
- dados ausentes forem importados sem duplicar registros;
- o cache Redis for usado na listagem/metadados/detalhe;
- o detalhe completar Pokémon incompleto ao ser acessado;
- a página de detalhe exibir informações principais, tipos, habilidades, movimentos, estatísticas e evoluções;
- a UI seguir os padrões existentes do projeto;
- testes ou validações mínimas forem adicionados conforme padrão do projeto.

delete daqui pra baixo 
    {
     "level_learned_at": 9,
     "move_learn_method": {
      "name": "level-up",
